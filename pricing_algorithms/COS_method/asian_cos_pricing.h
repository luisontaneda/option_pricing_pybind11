#include <Eigen/Dense>
#include <vector>
#include "cos_method.h"

using namespace std;


Eigen::MatrixXcd Clenshaw_Curtis_quadrature(double a, double b, int n_q, int N){
    ...
    auto   f_x	= [b, a] (double x, double u_k, double u_l) -> \
                            complex<double> {double temp_exp = (b-a)/2.0 * x + (a+b)/2.0;
                                            return (b-a)/2.0 * (pow(exp(temp_exp) + 1, i_c*u_k) * \
                                            cos((temp_exp - a)*u_l));};
...
double asian_cos_method(function<complex<double>(double)> c_f, double S_0,double K, double tau, double r, int time_steps, int N, int n_q, int L, double a, double b,string CP = "Call"){
    ...
    for (int k = 0; k < N; k++){
        ...
        c = a;
        d = log(K*(time_steps+1)/S_0 - 1);

        pair<double, double> chi_psi_pair = chi_psi(a, b, c, d, k);

        h_k = 2.0 / (b - a) * ((K - S_0/(time_steps+1))*chi_psi_pair.second - (S_0/(time_steps+1))*chi_psi_pair.first);
    ...
    if (CP == "Put"){
        ...
        double put_call_par = Opt_val + S_0*exp(-r*tau)/(time_steps+1) * (exp(r*dt*(time_steps+1)) - 1)/(exp(r*dt) - 1) - K*exp(-r*tau);
